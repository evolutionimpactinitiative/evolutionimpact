// File: app/api/donation-success/route.ts (Your existing file - updated)
  ...
  try {
    ...
    if (!existingDonation && session.payment_status === "paid") {
      ...
      try {
        ...
        // Update campaign totals with improved error handling
        if (campaignId) {
          await updateCampaignTotals(campaignId, amount, campaignTitle);
        }
...
async function updateCampaignTotals(
  ...
  try {
    ...
    const existingCampaign = await campaignsCollection.findOne({ campaignId });
    ...
    if (existingCampaign) {
      ...
      // Create new campaign with default goal of 10,000
      const newCampaign: CampaignDocument = {
        ...
        totalRaised: amount,
        totalDonations: 1,
        goal: 10000, // Default goal set to 10,000
        lastUpdated: new Date(),
        createdAt: new Date(),
      };
